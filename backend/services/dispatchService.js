...
const dispatchService = {
  async triggerAutomatedRiderDispatch(
    orderId,
    restaurantLocation,
    appIoContext,
  ) {
    try {
      ...
      if (riderQueueIds.length === 0) {
        console.warn(
          "[DISPATCH] No valid riders remained after queue sanitization.",
        );
        return null;
      }
      ...
      if (updatedOrder && appIoContext) {
        appIoContext.to(firstRiderId).emit("newOrderOffer", updatedOrder);
      }
...
dispatchService.advanceDispatchQueue = async function (orderId, appIoContext) {
  try {
    const order = await Order.findById(orderId)
      .select(
        "dispatchQueue currentDispatchIndex offeredRiderId assignedRiderId",
      )
      .populate("customerId", "name phone")
      .populate("restaurantId", "name address phone location");
