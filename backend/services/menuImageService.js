...
const buildMenuImageDirectory = (restaurantId, menuItemId) => {
  const safeRestaurantId = assertSafeIdentifier(restaurantId, "restaurant_id");
  const safeMenuItemId = assertSafeIdentifier(menuItemId, "menu_item_id");
...
const buildMenuImagePath = (restaurantId, menuItemId, filename) => {
  const directory = buildMenuImageDirectory(restaurantId, menuItemId);
...
const deleteMenuImage = async (restaurantId, menuItemId, filename) => {
  ...
  try {
    imagePath = buildMenuImagePath(restaurantId, menuItemId, filename);
  } catch {
    return;
  }
...
const resolveMenuImagePath = async (restaurantId, menuItemId, filename) => {
  const imagePath = buildMenuImagePath(restaurantId, menuItemId, filename);
