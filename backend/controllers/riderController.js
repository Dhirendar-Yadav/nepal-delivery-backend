  ...
  try {
    ...
    const getUrl = (fieldname) => {
      return files[fieldname]
        ? `${baseUrl}/uploads/${files[fieldname][0].filename}`
        : null;
    };
...
exports.getProfile = async (req, res) => {
  try {
    ...
    const mergedData = {
      ...
      licenseNumber:
        rider?.licenseNumber || profile?.licenseNumber || "Not Set",
      citizenshipNo:
        rider?.citizenshipNo || profile?.citizenshipNo || "Not Set",
      ...
      walletBalance: Math.max(0, Number(profile?.wallet?.balance || 0)),
      ...
      isVerified: user.kycStatus === "VERIFIED" || profile?.isVerified === true,
    };
...
exports.updateBike = async (req, res) => {
  try {
    if (!req.body.bikeNumber) {
      return res
        .status(400)
        .json({ success: false, message: "Bike number is required" });
    }
    ...
    if (bikeNumber.length > 20) {
      return res
        .status(400)
        .json({ success: false, message: "Bike number too long" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to update bike number" });
  }
};
...
exports.getAvailableOrders = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch available orders" });
  }
};
...
exports.getActiveOrder = async (req, res) => {
  try {
    ...
    const activeOrder = await Order.findOne({
      ...
      .populate(
        "restaurantId",
        "name location latitude longitude address phone",
      )
      .populate("customerId", "name phone address")
      .lean();
    ...
    if (activeOrder) {
      ...
      return res
        .status(200)
        .json({ success: false, message: "No active orders" });
    }
  } catch (err) {
    console.error("Get active order error:", err);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch active order" });
  }
};
...
exports.acceptOrder = async (req, res) => {
  ...
  try {
    ...
    if (!riderLocked) {
      ...
      return res
        .status(409)
        .json({ success: false, message: "You already have an active order" });
    }
    ...
    if (!order) {
      ...
      return res
        .status(409)
        .json({ success: false, message: "Order no longer available" });
    }
    ...
    if (riderLocked && riderId) {
      try {
        await User.findByIdAndUpdate(riderId, { currentActiveOrderId: null });
      } catch (unlockErr) {
...
exports.updateLocation = async (req, res) => {
  try {
    ...
    if (typeof latitude !== "number" || typeof longitude !== "number") {
      return res
        .status(400)
        .json({ success: false, message: "Invalid coordinate format" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Coordinates out of valid range" });
    }
    ...
    const currentOrder = await Order.findOne({
      ...
      .select("_id restaurantId status riderLocation riderOnTheWayAt")
      .lean();
    ...
    if (!currentOrder) {
      return res
        ...
        .json({
          success: false,
          message: "Active order not found or access denied",
        });
    }
    ...
    if (riderMovedOnTheWay) {
      ...
      const markedOnTheWayOrder = await Order.findOneAndUpdate(
        ...
        .select("_id restaurantId assignedRiderId riderOnTheWayAt")
        .lean();
...
exports.completeOrder = async (req, res) => {
  ...
  try {
    ...
    if (order.paymentMethod === "ONLINE" && order.paymentStatus !== "PAID") {
      ...
      return res
        ...
        .json({
          ...
          message: "Online payment must be completed before delivery",
        });
    }

    // STEP 2: Verify OTP using timing-safe method BEFORE any state mutation
    const isTokenValid = Order.verifyTimingSafeOTP(
      otp,
      order.deliveryOTP,
    ...
    if (!isTokenValid) {
      console.warn(
        `Incorrect OTP attempt - Order: ${orderId}, Rider: ${riderId}`,
      );
      await session.abortTransaction();
      session.endSession();
      return res
    ...
    if (!financialStatus.valid) {
      console.error(
        `Financial validation failed - Order: ${orderId}, Delta: ${financialStatus.delta}`,
      );
      await session.abortTransaction();
      session.endSession();
      return res
        .status(400)
        .json({ success: false, message: "Financial data mismatch" });
    }
    ...
    if (!updatedOrder) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Order was completed by another request",
        });
    }
    ...
    if (user && user.shiftStartTime && user.isOnline) {
      const shiftDurationMs =
        now.getTime() - new Date(user.shiftStartTime).getTime();
      const maxDurationMs =
        (process.env.MAX_SHIFT_DURATION_MINUTES || 480) * 60 * 1000;
      ...
      if (shiftDurationMs >= maxDurationMs) {
        ...
        forcedOffline = true;
        shiftEndedMsg =
          " Your shift has ended. Please log back in to start a new shift.";
      }
  ...
  try {
    ...
    if (typeof targetStatus !== "boolean") {
      return res
        ...
        .json({
          success: false,
          message: "Invalid targetStatus. Must be true or false",
        });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to update shift status" });
  }
};
