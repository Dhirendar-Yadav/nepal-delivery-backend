...
exports.getAllRestaurantsForAdmin = async (req, res, next) => {
  try {
    ...
    if (status) {
      if (typeof status !== "string") {
        return res
          ...
          .json({
            success: false,
            error:
              "Status parameter query attribute must be a valid string format",
          });
      }
      ...
      if (!ALLOWED_STATUSES.includes(upperStatus)) {
        return res
          ...
          .json({
            success: false,
            error: "Invalid status filter attribute provided",
          });
      }
      ...
      if (isDeleted !== "true" && isDeleted !== "false") {
        return res
          ...
          .json({
            success: false,
            error:
              "isDeleted parameter attribute value must be exactly 'true' or 'false'",
          });
      }
      ...
      if (typeof search !== "string") {
        return res
          ...
          .json({
            success: false,
            error:
              "Search query query parameter value must be a valid string primitive",
          });
      }
      if (search.length > 100) {
        return res
          ...
          .json({
            success: false,
            error:
              "Search query string parameter length limit exceeded max 100 characters",
          });
      }
    ...
    if (lastId) {
      if (!mongoose.Types.ObjectId.isValid(lastId)) {
        return res
          ...
          .json({
            success: false,
            error: "Invalid lastId cursor format signature",
          });
      }
      ...
      if (lastSortValue) {
        query.$or = [
          ...
          {
            createdAt: new Date(lastSortValue),
    ...
    const restaurants = await Restaurant.find(query)
      .select(
        "_id name status ownerId location foodType rating isOpen isDeleted isVerifiedByAdmin image registrationDoc panVatNumber createdAt __v",
      )
    ...
    const nextCursorId = hasNextDataPayload
      ? restaurants[restaurants.length - 1]._id
      : null;
    const nextCursorSortValue = hasNextDataPayload
      ? restaurants[restaurants.length - 1].createdAt
      : null;
    ...
    console.error(
      "Critical error inside getAllRestaurantsForAdmin database read engine:",
      error,
    );
    return res
      .status(500)
      .json({
        success: false,
        error:
          "Internal server error during data retrieval pipeline processing",
      });
  }
};
...
exports.updateRestaurantStatus = async (req, res, next) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res
      ...
      .json({
        success: false,
        error: "Invalid Restaurant ID format structure reference",
      });
  }
  ...
  if (status !== undefined) {
    if (typeof status !== "string") {
      return res
        ...
        .json({
          success: false,
          error:
            "Status mutation parameter attribute must evaluate to a valid string primitive",
        });
    }
    ...
    if (!ALLOWED_STATUSES.includes(targetStatusScope)) {
      return res
        ...
        .json({
          success: false,
          error: `Invalid status setup configuration state parameter. Scopes allowed: ${ALLOWED_STATUSES.join(", ")}`,
        });
    }
    ...
    if (typeof isVerifiedByAdmin !== "boolean") {
      return res
        ...
        .json({
          success: false,
          error:
            "isVerifiedByAdmin parameters must map to a raw native boolean primitive flag",
        });
    }
  ...
  if (Object.keys(updateFields).length === 0) {
    return res
      ...
      .json({
        success: false,
        error: "No valid structural update parameter metrics supplied",
      });
  }
  ...
  try {
    ...
    if (!updatedRestaurant) {
      const errorInstance = new Error(
        "Concurrency Lock Failure: Target record state modified or client data stale. Operation aborted safely.",
      );
    ...
    const ownerAccount = await User.findById(updatedRestaurant.ownerId).session(
      session,
    );
    if (!ownerAccount) {
      const errorInstance = new Error(
        "Dangling Integrity Refusal: Associated restaurant owner account structure missing or purged",
      );
    ...
    if (updateFields.status) {
      switch (updateFields.status) {
        ...
        case "REJECTED":
          await User.findByIdAndUpdate(
            updatedRestaurant.ownerId,
            { $set: { isActive: false, kycStatus: "REJECTED" } },
            { session, runValidators: true },
          );
          break;
        case "PENDING":
    ...
    console.error(
      "Administrative update transaction cluster execution aborted and state changes rolled back safely:",
      error,
    );
    return res
      ...
      .json({
        ...
        error:
          error.message ||
          "Internal server error during modification cascade execution",
      });
  } finally {
    session.endSession();
  }
  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        ...
        .json({
          success: false,
          error: "Invalid Restaurant ID format structure",
        });
    }
      ...
      if (typeof isOpen !== "boolean") {
        return res
          ...
          .json({
            success: false,
            error:
              "isOpen target parameter parameter field input must evaluate to a strict primitive boolean value",
          });
      }
      ...
      if (typeof isDeleted !== "boolean") {
        return res
          ...
          .json({
            success: false,
            error:
              "isDeleted target parameter parameter field input must evaluate to a strict primitive boolean value",
          });
      }
    ...
    if (Object.keys(updateFields).length === 0) {
      return res
        ...
        .json({
          success: false,
          error: "No valid operational modification parameters specified",
        });
    }
    ...
    if (!updatedRestaurant) {
      return res
        .status(404)
        .json({ success: false, error: "Restaurant record not located" });
    }

    return res.status(200).json({ success: true, data: updatedRestaurant });
  } catch (error) {
    console.error(
      "Error tracked inside updateOperationalState execution block context:",
      error,
    );
...
exports.updateRankingMetrics = async (req, res, next) => {
  try {
    ...
    const validateStrictNumber = (value, attributeName) => {
      if (value !== undefined) {
        if (typeof value === "boolean") {
          throw new Error(
            `${attributeName} parameter configuration layer cannot evaluate to a raw boolean value primitive`,
          );
        }
        if (typeof value === "string" && value.trim() === "") {
          throw new Error(
            `${attributeName} input token data sequence cannot evaluate to empty space matrix fields`,
          );
        }
        const parsedValue = Number(value);
        if (!Number.isFinite(parsedValue)) {
          throw new Error(
            `${attributeName} attributes parameters configurations must map to finite numeric representations`,
          );
        }
        return parsedValue;
      }
    ...
    const checkedDeliveryTime = validateStrictNumber(
      avgDeliveryTime,
      "avgDeliveryTime",
    );
      ...
      if (checkedDeliveryTime < 0) {
        return res
          ...
          .json({
            success: false,
            error:
              "Logical Error: avgDeliveryTime metric parameter fields cannot fall below zero metrics boundaries",
          });
      }
      updateFields.avgDeliveryTime = checkedDeliveryTime;
    }

    const checkedCommissionRate = validateStrictNumber(
      commissionRate,
      "commissionRate",
    );
      ...
      if (checkedCommissionRate < 0 || checkedCommissionRate > 100) {
        return res
          ...
          .json({
            success: false,
            error:
              "Logical Error: commissionRate parameters constraints must reside bounded tightly inside 0 to 100 percentage parameters",
          });
      }
      ...
      if (typeof offerTag !== "string") {
        return res
          ...
          .json({
            success: false,
            error:
              "offerTag configuration parameters input must evaluate to a strict primitive string format",
          });
      }
    ...
    if (Object.keys(updateFields).length === 0) {
      return res
        ...
        .json({
          success: false,
          error:
            "No valid intentional parameter updates configured inside metrics data execution matrix",
        });
    }
    ...
    console.error(
      "Error tracked inside updateRankingMetrics execution block context:",
      error,
    );
...
exports.syncLegacyData = async (req, res, next) => {
  try {
    ...
    const restrictedOwnerIdSet = new Set(
      restrictedPartners
        .map((p) => (p.ownerId ? p.ownerId.toString() : null))
        .filter(Boolean),
    );

    const restaurantCursor = Restaurant.find({ status: "ACTIVE" }, "ownerId")
      .lean()
      .cursor();
    ...
    const flushBatchProcessStream = async (idsArray) => {
      ...
      const eligibleOwnerIds = uniqueOwnerIds.filter(
        (ownerId) => !restrictedOwnerIdSet.has(ownerId),
      );
    ...
    for (
      let restDoc = await restaurantCursor.next();
      restDoc != null;
      restDoc = await restaurantCursor.next()
    ) {
      if (restDoc.ownerId) {
      ...
      if (batchOwnerIds.length >= BATCH_PROCESSING_LIMIT_SIZE) {
        totalProcessedAccountsCount +=
          await flushBatchProcessStream(batchOwnerIds);
        batchOwnerIds = [];
      }
    }
    ...
    console.error(
      "Critical crash tracked inside syncLegacyData background batch processing streaming execution layer:",
      error,
    );
    return res
      .status(500)
      .json({
        success: false,
        error:
          "Internal server error occurred inside background data stream processing engines",
      });
  }
};
