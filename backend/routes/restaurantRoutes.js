...
const maskSensitiveValue = (value, visibleStart = 0, visibleEnd = 4) => {
  ...
  const prefix = visibleStart > 0 ? normalized.slice(0, visibleStart) : "";
  ...
  return {
    ...
    eSewaId:
      method === "eSewa"
        ...
        : null,
    eSewaAccountName:
      method === "eSewa" ? payoutSettings.eSewaAccountName || null : null,
    bankDetails:
      method === "Bank"
        ? {
            accountName: bankDetails.accountName || null,
            bankName: bankDetails.bankName || null,
            accountNumber: maskSensitiveValue(bankDetails.accountNumber, 0, 4),
          }
        : null,
  };
};
  ...
  authMiddleware(req, res, async () => {
    if (req.user.role !== "Seller") {
      return res.status(403).json({
        ...
        message:
          "Restricted Access! Seller authorization clearance flags required.",
      });
    }
    ...
    if (
      ...
      return res.status(403).json({
        ...
        message:
          "Access Suspended: This account has been deactivated or failed verification checks.",
      });
    }
...
const attachRestaurantContext = asyncHandler(async (req, res, next) => {
  const restaurant = await Restaurant.findOne({ ownerId: req.user.id })
    .select(
      "_id currentLocation name isOpen status image registrationDoc panVatNumber",
    )
    .lean();
  ...
  if (!restaurant) {
    return res
      ...
      .json({
        ...
        message:
          "Operational block: Seller account possesses no active restaurant profile setup mapping entries.",
      });
  }
  ...
  if (restaurant.status !== "ACTIVE") {
    return res
      ...
      .json({
        ...
        message:
          "Access Forbidden: Restaurant profiling has been suspended or is currently unapproved.",
      });
  }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(restaurantId)) {
      return res
        .status(400)
        .json({ success: false, error: "INVALID_RESTAURANT_ID" });
    }
    ...
    if (!restaurant?.image) {
      return res
        .status(404)
        .json({ success: false, error: "RESTAURANT_IMAGE_NOT_FOUND" });
    }
    ...
    const candidatePaths = [managedImagePath, legacyImagePath];
  ...
  asyncHandler(async (req, res) => {
    ...
    return res.json({
      ...
      restaurant: {
        ...
        payoutSettings: buildSafePayoutSettings(restaurant.payoutSettings),
      },
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const validation = validatePaymentMethodPayload(method, req.body || {});
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const lifetimeLedgerReceived = Number(
      lifetimeSummaryData.totalReceived || 0,
    );
    ...
    const expectedPendingBalance = lifetimeTotalEarned - totalSettled;
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updatedItem = await MenuItem.findOneAndUpdate(
      buildMenuItemMutationFilter(req.restaurant._id, id, version),
      {
        $set: updateFields,
        $inc: { __v: 1 },
      },
    ...
    if (!updatedItem) {
      ...
      return res.status(409).json({
        ...
        message:
          "Menu item was changed by another request. Refresh the item and try again.",
      });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      storedFilename = await storeMenuImage(req.file, req.restaurant._id, id);
      ...
      if (!updatedItem) {
        await deleteMenuImage(req.restaurant._id, id, storedFilename);
      ...
      if (currentItem.image && currentItem.image !== storedFilename) {
        try {
          await deleteMenuImage(req.restaurant._id, id, currentItem.image);
        } catch (cleanupError) {
          if (req.log) {
            req.log.warn({
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (deletedItem.image) {
      try {
        await deleteMenuImage(req.restaurant._id, id, deletedItem.image);
      } catch (cleanupError) {
        if (req.log) {
          req.log.warn({
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(orderId)) {
      return res
        ...
        .json({
          ...
          message:
            "Malformed context parameter reference format identifier dropped.",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          ...
          message:
            "Requested state out of system runtime parameters limits mappings bounds.",
        });
    }
    ...
    const existingOrder = await Order.findOne({
      ...
      .select(
        "status offeredRiderId assignedRiderId paymentMethod paymentStatus",
      )
      .lean();
    if (!existingOrder) {
      return res
        ...
        .json({
          ...
          message:
            "Target request dataset criteria matches not found inside databases collections.",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          ...
          message: "Online payment must be completed before order processing.",
        });
    }
    ...
    if (!order) {
      return res
        ...
        .json({
          ...
          error: "CONCURRENCY_CONFLICT",
          message:
            "State Mutation Blocked: Target transaction version hijacked by concurrent processes.",
        });
    }
        ...
        if (appIoContext) {
          appIoContext
            ...
            .emit("foodReadyForPickup", {
              ...
              restaurantName:
                order.restaurantId?.name || req.restaurant.name || "Restaurant",
              status: order.status,
    ...
    if (DISPATCH_TRIGGER_STATUSES.includes(status)) {
      ...
      setImmediate(async () => {
        const appIoContext = req.app.get("io");
        await dispatchService.triggerAutomatedRiderDispatch(
          order._id,
          req.restaurant.currentLocation,
          appIoContext,
        );
      });
    }
