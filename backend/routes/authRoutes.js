...
const loginLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: "Too many login attempts. System locked for 15 minutes.",
  },
});

const DUMMY_HASH =
  "$2a$12$C6UzMDM.H6dfI/f/IKcEeO6GZ5z6uGq5t1k8Kp1l9z0h9fQp7q9aW";
...
router.post("/signup", upload.any(), async (req, res) => {
  ...
  try {
    ...
    if (!phone || !/^\d{10,15}$/.test(phone))
      throw {
        status: 400,
        message: "Valid phone number (10-15 digits) is required.",
      };
    ...
    const userCoordinates = hasValidCoordinates ? [lng, lat] : [0, 0];
    ...
    if (role === "Seller") {
      ...
      const registrationDocFile = req.files?.find(
        (file) => file.fieldname === "registrationDoc",
      );
    ...
    if (req.log)
      req.log.info({ event: "USER_SIGNUP_SUCCESS", userId: newUser._id, role });
    ...
    if (role === "Seller") {
      return res
        ...
        .json({
          success: true,
          message: "Seller account created! Please wait for Admin approval.",
        });
    }
    res
      .status(201)
    ...
    if (req.log)
      req.log.error({ event: "USER_SIGNUP_FAILED", error: err.message });
...
router.post("/rider/signup", upload.any(), async (req, res) => {
  ...
  try {
    ...
    if (Rider) {
      ...
      console.warn(
        "⚠️ Rider model not imported/created. Rider specific details not saved.",
      );
    }
    ...
    if (req.log)
      req.log.info({ event: "RIDER_SIGNUP_SUCCESS", userId: newUser._id });
    ...
    if (req.log)
      req.log.error({ event: "RIDER_SIGNUP_FAILED", error: err.message });
...
router.post("/login", loginLimiter, async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }).select(
      "+password +isActive +isBlocked +kycStatus +role +name +phone",
    );
    ...
    if (!user.isActive || user.isBlocked) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Account is disabled or pending Admin approval.",
        });
    }
    ...
    if (req.log)
      req.log.error({ event: "LOGIN_SYSTEM_ERROR", error: err.message });
...
router.get("/me", authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select(
      "_id name email phone role isActive isBlocked isDeleted kycStatus profileImage",
    );
  ...
  async (req, res) => {
    ...
    try {
      ...
      const user = await User.findOne({
        ...
        .select("_id profileImage isActive isBlocked isDeleted kycStatus")
        .lean();
      ...
      if (!updatedUser) {
        ...
        return res.status(409).json({
          ...
          message:
            "Profile photo was updated by another request. Please try again.",
        });
      }
...
router.get("/profile/photo", authMiddleware, async (req, res) => {
  try {
    ...
    const profilePath = path.resolve(profileDirectory, filename);
