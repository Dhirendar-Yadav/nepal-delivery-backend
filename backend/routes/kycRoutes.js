...
router.get("/documents/:filename", authMiddleware, async (req, res, next) => {
  ...
  log.info({
    event: "KYC_DOCUMENT_ACCESS_ATTEMPT",
    userId: req.user.id,
    filename,
  });
  ...
  if (
    ...
    log.warn({
      event: "KYC_DOCUMENT_ACCESS_DENIED",
      userId: req.user.id,
      reason: "INVALID_FILENAME",
    });
    return res
      .status(400)
      .json({ success: false, error: "INVALID_DOCUMENT_PATH" });
  }
  ...
  try {
    const legacyDocumentPattern = new RegExp(
      `${escapeRegex(`/uploads/${filename}`)}$`,
    );
    ...
    if (rider) {
      ...
      const riderDocumentField = [
        ...
        (field) =>
          normalizeStoredFilename(rider.documents?.[field]) === filename,
      );
      ...
      const restaurant = await Restaurant.findOne({
        $or: [{ image: documentValues }, { registrationDoc: documentValues }],
      })
        .select("ownerId image registrationDoc")
        .lean();
      ...
      if (restaurant) {
        const documentType =
          normalizeStoredFilename(restaurant.registrationDoc) === filename
            ? "registrationDoc"
    ...
    if (req.user.role !== "Admin" && ownerId.toString() !== req.user.id) {
      ...
      return res
        .status(403)
        .json({ success: false, error: "UNAUTHORIZED_DOCUMENT_ACCESS" });
    }

    const legacyDocumentPath = path.resolve(uploadDirectory, filename);
    const candidatePaths = [managedDocumentPath, legacyDocumentPath].filter(
      Boolean,
    );
