...
const requireRider = (req, res, next) => {
  if (req.user.role !== "Rider") {
    return res
      .status(403)
      .json({ success: false, message: "Access restricted to Riders only." });
  }
  next();
};
...
router.get(
  ...
  authMiddleware,
  requireRider,
  riderController.getActiveOrder,
);
router.post(
  "/update-location",
  authMiddleware,
...
router.put(
  ...
  authMiddleware,
  requireRider,
  riderController.rejectOrder,
);
router.put(
  "/orders/:id/complete",
  authMiddleware,
  requireRider,
  riderController.completeOrder,
);
