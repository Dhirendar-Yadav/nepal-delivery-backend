...
const BulkPayoutFailure =
  mongoose.models.BulkPayoutFailure ||
  mongoose.model(
    "BulkPayoutFailure",
    new mongoose.Schema({
      batchId: String,
...
const BulkPayoutBatch =
  mongoose.models.BulkPayoutBatch ||
  mongoose.model(
    "BulkPayoutBatch",
    new mongoose.Schema({
      _id: String,
...
const SystemLock =
  ...
  mongoose.model(
    ...
    new mongoose.Schema({
      ...
      isLocked: Boolean,
      ownerId: String,
      lockedAt: { type: Date, default: Date.now, expires: 300 },
    }),
  );
...
router.get("/financial-hub", verifyAdmin, criticalLimiter, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        pendingRiderPayouts: riders,
        pendingSellerSettlements: sellers,
        masterWallet: wallet,
      },
    });
  } catch (err) {
...
router.post(
  "/restaurants/:id/settle",
  verifyAdmin,
  criticalLimiter,
  financeController.processRestaurantSettlement,
);
    ...
    try {
      ...
      if (!acquireLock || acquireLock.ownerId !== ownerSignature) {
        return res
          ...
          .json({
            success: false,
            message: "Batch is currently being processed by another instance.",
          });
      }
      ...
      try {
        await AdminAuditLog.create({
          _id: chunkHash,
          adminId: req.user.id,
          action: "BULK_PAYOUT_CHUNK",
          targetType,
          batchId,
          cursor: lastId || null,
        });
        await BulkPayoutBatch.findOneAndUpdate(
          { _id: batchId },
          { $setOnInsert: { targetType, status: "PROCESSING" } },
          { upsert: true },
        );
      } catch (err) {
        ...
        if (err.code === 11000)
          return res
            ...
            .json({
              success: true,
              message: "Idempotent: Chunk already processed.",
            });
        throw err;
      }
      ...
      for (const entity of entities) {
        ...
        try {
          ...
          const ledgerEntityType =
            targetType === "RIDER" ? "RIDER" : "RESTAURANT";
          const ledgerEntityId =
            targetType === "RIDER" ? entity.userId : entity._id;
          ...
          const ledgerEntries = [
            {
              ...
              amount: payoutAmount,
              balanceAfter:
                targetType === "SELLER" ? updatedEntity.walletBalance : null,
      ...
      let finalBatchStatus =
        entities.length < 50
          ? failedCount === 0
            ? "COMPLETED"
            : "PARTIAL"
          : "PROCESSING";
      await BulkPayoutBatch.updateOne(
        { _id: batchId },
        {
          $inc: {
      ...
      res.json({
        ...
        nextCursor: entities.length === 50 ? nextCursor : null,
      });
    } catch (err) {
