...
function computeDiscoverable(doc) {
  ...
  const lastActive = doc.lastActiveAt
    ? new Date(doc.lastActiveAt).getTime()
    : Date.now();
...
const restaurantSchema = new mongoose.Schema(
  {
    ...
    walletBalance: {
      ...
      validate: {
        ...
        message: "Balance must be in Paisa (Integer)",
      },
    },
    totalEarnings: {
      type: Number,
      ...
      validate: {
        ...
        message: "Earnings must be integer paisa.",
      },
    },
    totalSettled: {
      type: Number,
      ...
      validate: {
        validator: Number.isSafeInteger,
        message: "Settled amount must be integer paisa.",
      },
    },
...
restaurantSchema.pre("save", function () {
  ...
  if (
    ...
    (!this.currentLocation || !this.currentLocation.coordinates)
  ) {
  ...
  if (this.foodTypes && this.foodTypes.length > 0) {
    this.foodTypes = [
      ...new Set(this.foodTypes.map((type) => type.toLowerCase().trim())),
    ];
  }
  ...
  if (this.totalRatings > 0) {
    this.rating =
      Math.round((this.totalRatingSum / this.totalRatings) * 10) / 10;
  } else {
    this.rating = 0;
  }
...
restaurantSchema.pre(["findOneAndUpdate", "updateOne"], async function (next) {
  ...
  if (
    update.$set &&
    Object.keys(update.$set).some((k) => k.startsWith("wallet"))
  ) {
    return next(
      new Error("CRITICAL: Wallet fields must use controlled atomic updates."),
    );
  }

  if (
    update.$unset &&
    Object.keys(update.$unset).some((k) => k.startsWith("wallet"))
  ) {
  ...
  if (
    update.$rename &&
    Object.keys(update.$rename).some((k) => k.startsWith("wallet"))
  ) {
  ...
  if (update.$inc) {
    ...
    const walletFields = ["walletBalance", "totalEarnings", "totalSettled"];

    const invalid = Object.keys(update.$inc).filter(
      (k) => walletFields.includes(k) && !allowedWalletInc.has(k),
    );
  ...
  if (update.$set || update.$inc) {
    ...
    const shouldRecomputeVisibility =
      update.$set && visibilityFields.some((f) => update.$set[f] !== undefined);
    const shouldRecomputeRating =
      (update.$set && ratingFields.some((f) => update.$set[f] !== undefined)) ||
      (update.$inc && ratingFields.some((f) => update.$inc[f] !== undefined));
    const shouldRecomputePopularity =
      shouldRecomputeRating ||
      (update.$set &&
        popularityFields.some((f) => update.$set[f] !== undefined)) ||
      (update.$inc &&
        popularityFields.some((f) => update.$inc[f] !== undefined));

    // 🔥 SINGLE FETCH OPTIMIZATION
    if (
      shouldRecomputeVisibility ||
      shouldRecomputeRating ||
      shouldRecomputePopularity
    ) {
      const doc = await this.model.findOne(this.getQuery()).lean();
      console.log(
        "HAS isPaused:",
        Object.prototype.hasOwnProperty.call(doc, "isPaused"),
      );
      ...
      if (doc) {
        ...
        if (shouldRecomputeRating) {
          ...
          if (mergedDoc.totalRatings > 0) {
            update.$set.rating =
              Math.round(
                (mergedDoc.totalRatingSum / mergedDoc.totalRatings) * 10,
              ) / 10;
          } else {
            update.$set.rating = 0;
          }
...
restaurantSchema.statics.assertFinancialInvariant = function (restaurant) {
  ...
  if (
    ...
    throw new Error(
      "Restaurant financial values must be safe integer paisa amounts.",
    );
  }
...
restaurantSchema.index(
  { name: "text", foodTypes: "text" },
  { weights: { name: 5, foodTypes: 3 } },
);
