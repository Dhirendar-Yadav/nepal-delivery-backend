    ...
    wallet: {
      balance: {
        ...
        validate: {
          ...
          message: "Balance must be integer paisa",
        },
      },
...
riderProfileSchema.pre(
  ...
  function (next) {
    ...
    if (
      ...
      return next(
        new Error(
          "CRITICAL: Wallet fields must use controlled atomic updates.",
        ),
      );
    }
    ...
    if (update?.$inc) {
      ...
      const invalid = Object.keys(update.$inc).filter(
        (k) => k.startsWith("wallet") && !allowedWalletInc.has(k),
      );
