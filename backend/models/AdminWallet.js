...
const adminWalletSchema = new mongoose.Schema(
  {
    ...
    totalPlatformRevenue: {
      ...
      validate: {
        ...
        message: "Revenue must be an integer (paisa).",
      },
    },
    ...
    totalRiderBonusesPaid: {
      ...
      validate: {
        validator: Number.isInteger,
        message: "Bonus must be an integer (paisa).",
      },
    },
...
adminWalletSchema.pre(
  ...
  async function (next) {
    ...
    if (
      ...
      return next(
        new Error(
          "CRITICAL: Financial fields must use controlled atomic updates.",
        ),
      );
    }
    ...
    if (
      update.$rename &&
      Object.keys(update.$rename).some(
        (k) =>
          k.startsWith("total") || k === "transactionCount" || k === "version",
      )
    ) {
