...
const orderSchema = new mongoose.Schema(
  {
    ...
    dispatchQueue: {
      ...
      validate: {
        ...
        message:
          "Dispatch Invariant Violation: Array queue buffer bounds cannot exceed 100 candidate items.",
      },
    },
    ...
    items: [
      {
        ...
        price: {
          ...
          validate: {
            ...
            message: "Price must be a Safe Paisa Integer.",
          },
        },
    ...
    deliveryDetails: {
      ...
      phone: {
        ...
        match: [
          /^\+?[0-9]{10,15}$/,
          "Please provide a valid phone number structure.",
        ],
      },
...
const getSystemPepperToken = () =>
  process.env.OTP_SECRET ||
  "FoodSamundarSystemServerPepperStringSignatureHashToken";

orderSchema.statics.hashOTP = function (otp) {
  if (otp == null || String(otp).trim() === "") return null;
  return crypto
    .createHash("sha256")
    .update(String(otp).trim() + getSystemPepperToken())
    .digest("hex");
};

orderSchema.statics.verifyTimingSafeOTP = function (
  rawOtp,
  storedHash,
  expiresAt,
  otpUsed,
  ...
  const incomingHash = crypto
    .createHash("sha256")
    .update(String(rawOtp).trim() + getSystemPepperToken())
    .digest();
 ...
 */
orderSchema.statics.buildStatusUpdatePayload = function (
  fromStatus,
  toStatus,
  actorType,
  actorId = null,
  ...
  if (!VALID_ORDER_STATUSES.includes(fromStatus)) {
    throw new Error(
      `State Machine Boundary Violation: Provided fromStatus [${fromStatus}] is not a recognized state machine entry metric.`,
    );
  }
  if (!VALID_ORDER_STATUSES.includes(toStatus)) {
    throw new Error(
      `State Machine Boundary Violation: Provided toStatus [${toStatus}] is not a recognized state machine entry metric.`,
    );
  }
  ...
  if (!VALID_ACTORS.includes(normalizedActor)) {
    throw new Error(
      `State Machine Security Exception: Provided actorType [${normalizedActor}] falls outside legal registration scopes: ${VALID_ACTORS.join(", ")}`,
    );
  }

  if (normalizedActor === "SYSTEM" && actorId != null) {
    throw new Error(
      "Operational Invariant Breach: Automated SYSTEM routines cannot retain human actorId contextual tracking assignments references.",
    );
  }
  ...
  if (!allowedTransitions.includes(toStatus)) {
    throw new Error(
      `State Machine Refusal Exception: Status transition track path from [${fromStatus}] to [${toStatus}] configuration is strictly illegal.`,
    );
  }
  ...
  if (normalizedActor !== "SYSTEM") {
    if (!actorId || !mongoose.Types.ObjectId.isValid(actorId)) {
      throw new Error(
        `State Machine Context Exception: Real human user operations require a valid, non-null structural ObjectId representation.`,
      );
    }
  }
 ...
 */
orderSchema.statics.buildNextDispatchPayload = function (
  dispatchQueue,
  currentIndex,
) {
  if (!Array.isArray(dispatchQueue)) {
    throw new Error(
      "Dispatch Engine Core Error: Target dispatch validation parameters queue is malformed.",
    );
  }

  if (currentIndex < -1 || currentIndex >= dispatchQueue.length) {
    throw new Error(
      `Dispatch State Invariant Corruption: Received dispatch index [${currentIndex}] drifts entirely out of bounds for current queue allocations array size [${dispatchQueue.length}].`,
    );
  }
 ...
 */
orderSchema.statics.validateFinancialBreakdown = function (
  financials,
  ...
  for (const key of keys) {
    ...
    if (
      ...
      throw new Error(
        `Financial Integrity Violation: Accounting data property key [${key}] must parse strictly as a valid finite safe integer primitive (Paisa).`,
      );
    }
  }
  ...
  const expectedTotal =
    foodCost +
    deliveryFee +
    (includePlatformFee ? platformFee : 0) +
    taxAmount -
    discountAmount;
