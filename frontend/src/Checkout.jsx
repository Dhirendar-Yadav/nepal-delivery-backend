    ...
    <div className="min-h-screen bg-[#FDF2F0] p-4 md:p-8 font-sans flex flex-col items-center justify-center">
      <div className="w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-orange-100 animate-slide-up">
        <div className="bg-gray-900 p-8 text-center text-white">
          ...
          <p className="font-bold text-gray-400 mt-2 tracking-widest uppercase text-[10px]">
        ...
        <div className="p-8 flex flex-col items-center">
          ...
          <p className="text-xs font-bold text-gray-400 mb-8 text-center uppercase tracking-widest">
            ...
            {order.status === "Accepted" &&
              "Restaurant accepted! Finding a rider..."}
            {order.status === "Confirmed" &&
              "Restaurant confirmed! Finding a rider..."}
          ...
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 w-full rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            {/*SECURITY FIX: Only show OTP when status is Out for Delivery */}
            ...
            <p className="text-[10px] font-bold text-gray-400 mt-5 text-center px-4">
              Share this PIN with the rider{" "}
              <span className="text-red-500 font-black">ONLY</span> when they
              deliver your food.
            </p>
          </div>
            ...
            <div className="mt-8 w-full bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center justify-between shadow-sm">
              <div>
                ...
                <p className="font-black text-blue-900 text-lg">
                  {order.assignedRiderId.name}
                </p>
                {order.assignedRiderId.bikeNumber && (
                  <p className="text-[10px] font-bold text-blue-600">
                    Bike: {order.assignedRiderId.bikeNumber}
                  </p>
                )}
              </div>
              <a
                href={`tel:${order.assignedRiderId.phone}`}
                className="bg-blue-600 text-white w-12 h-12 flex items-center justify-center rounded-full font-bold shadow-md hover:bg-blue-700 transition-all text-xl active:scale-95"
              ></a>
            </div>
          )}
          ...
          <button
            onClick={() => navigate("/")}
            className="mt-10 text-gray-400 font-black text-xs uppercase tracking-widest hover:text-orange-500 transition-colors"
          >
            Back to Home
          </button>
        </div>
...
function Checkout() {
  ...
  const [position, setPosition] = useState([27.502, 83.6661]);
  ...
  const distanceKm = useMemo(() => {
    ...
    const lat1 = restaurantLatitude;
    const lon1 = restaurantLongitude;
  ...
  }, [position, restaurantLatitude, restaurantLongitude]);
  ...
  const deliveryFee = useMemo(() => {
    ...
    const calculatedFee = (fuelCostPerKM + riderProfitPerKM) * distanceKm;

    return calculatedFee < 25 ? 25 : Math.round(calculatedFee);
  }, [distanceKm, petrolPrice]);

  const grandTotal = foodTotal + deliveryFee;

  // FIX: Added AbortController to prevent race conditions during reverse geocoding
  ...
  // FIX: Debouncing implementation for Map Dragging (Prevents API Ban)
  useEffect(() => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!restaurantId) {
      alert(
        "Error: Restaurant ID is missing! Please go back to the cart and try again.",
      );
      return;
    }

    if (
      address === "Locating your hunger..." ||
      (position[0] === 27.502 && position[1] === 83.6661)
    ) {
      alert(
        "Please select or confirm your delivery location before placing the order.",
      );
      return;
    }
    ...
    try {
      const attempt = pendingCheckout?.attemptId
        ? pendingCheckout
        : beginCheckoutAttempt();
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#FDF2F0] p-4 font-sans">
        <div className="bg-white rounded-[2.5rem] p-10 text-center shadow-2xl border border-orange-50">
          ...
          <button
            onClick={() => navigate("/")}
            className="mt-8 bg-orange-500 hover:bg-gray-900 text-white font-black py-4 px-8 rounded-2xl transition-all"
          >
            Back to Home
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#FDF2F0] p-4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-gray-400 hover:text-orange-500 font-black text-sm uppercase transition-all group"
          >
          ...
          <div className="p-6 md:p-10">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <button
                ...
                className={`p-4 rounded-2xl border-2 transition-all font-bold ${paymentMethod === "COD" ? "border-orange-500 bg-orange-50" : "border-gray-100"}`}
              >
                COD
              </button>
              <button
                onClick={() => setPaymentMethod("Online")}
                className={`p-4 rounded-2xl border-2 transition-all font-bold ${paymentMethod === "Online" ? "border-orange-500 bg-orange-50" : "border-gray-100"}`}
              >
                Online
              </button>
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-black text-sm transition-all mb-4 uppercase ${isLocating ? "bg-gray-400" : "bg-gray-900 text-white hover:bg-orange-500"}`}
            >
            ...
            <div
              ...
              className="rounded-2xl overflow-hidden border-2 border-gray-100 z-0"
            >
              <MapContainer
                center={position}
                zoom={18}
            ...
            <button
              ...
              className={`w-full mt-8 py-6 rounded-2xl font-black text-xl transition-all uppercase ${isPlacingOrder ? "bg-gray-300" : "bg-orange-500 hover:bg-gray-900 text-white shadow-xl shadow-orange-200"}`}
            >
