...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    if (!phoneRegex.test(formData.phone)) {
      alert(
        "⚠️ Security Alert: Kripya sahi Nepali mobile number (10 digits, starting with 9) halnuhos.",
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        alert(
          "Swagat Chha! 🥘 Account created successfully. Login garera mitho khana order garnuhos.",
        );
        navigate("/login");
      } else {
      ...
      <div className="w-full min-h-screen flex flex-col items-center justify-center p-6 sm:p-12 lg:p-20">
        ...
        <div className="w-full max-w-4xl flex flex-col">
          <div className="text-center mb-16 mt-16 md:mt-0">
            <div className="inline-block bg-orange-50 p-6 rounded-[2.5rem] mb-8 text-6xl shadow-inner">
              🥘
            </div>
          ...
          <form onSubmit={handleSignup} className="flex flex-col gap-10">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="flex flex-col gap-3">
                <label className="text-sm font-black text-orange-500 uppercase tracking-widest ml-1">
                  Full Name (Tapai ko Nam)
                </label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-0 shadow-inner rounded-[2rem] overflow-hidden border-2 border-transparent focus-within:border-orange-500 transition-all">
                  <span className="bg-gray-100 px-6 flex items-center text-lg font-black text-gray-400">
                    +977
                  </span>
                  <input
                    type="tel"
