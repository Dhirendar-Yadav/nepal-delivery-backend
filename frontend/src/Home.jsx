...
function Home() {
  ...
  const { restaurants, isLoading, apiError } = useRestaurants();
  ...
  const { dynamicSidebarCategories } = useDynamicCategories(restaurants);
  ...
  // CEO Profile Section
  if (showCEOProfile) {
    return <CEOProfile onClose={() => setShowCEOProfile(false)} />;
  }
