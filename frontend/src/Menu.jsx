...
function Menu() {
  ...
  const handleCheckout = () => {
    ...
    navigate("/checkout", {
      state: { cartItems: cart.items, restaurantId: id, totalAmount },
    });
  };
    ...
    <div className="w-full min-h-screen bg-[#F8FAFC] font-sans text-gray-900 m-0 p-0 overflow-x-hidden">
      ...
      <nav className="w-full bg-white/70 backdrop-blur-lg sticky top-0 z-50 border-b border-gray-200">
        <div className="max-w-[1400px] mx-auto px-6 py-4 flex justify-between items-center">
          <Link
            to="/"
            className="bg-gray-100 hover:bg-orange-500 hover:text-white px-4 py-2 rounded-2xl font-bold transition-all duration-300"
          >
            ⬅ Back
          </Link>
          <div className="text-center">
            <p className="text-[10px] font-black text-orange-500 uppercase tracking-[3px]">
              Ordering From
            </p>
            <h1 className="text-xl md:text-2xl font-black text-gray-900 leading-none">
              {restaurantName || "Food Samundar"}
            </h1>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {menuItems.map((item) => (
                  <div
                    key={item._id}
                    className="bg-white rounded-[2.5rem] p-8 shadow-[0_15px_50px_-15px_rgba(0,0,0,0.05)] border border-gray-50 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden"
                  >
                    ...
                    <span className="bg-green-100 text-green-700 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-tighter">
                      {item.foodCategory || "Not Specified"}
                    </span>

                    <h3 className="text-2xl font-black text-gray-800 mt-6 group-hover:text-orange-600 transition-colors">
                      {item.name}
                    </h3>
                    <p className="text-gray-400 text-sm mt-3 leading-relaxed min-h-[50px]">
                      {item.description ||
                        "Freshly cooked and packed with authentic Nepali spices and lots of love."}
                    </p>
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-white rounded-[3rem] p-8 shadow-[0_30px_100px_rgba(0,0,0,0.1)] border border-gray-50 sticky top-32">
              <h3 className="text-2xl font-black mb-8 text-gray-900 flex justify-between items-center">
                Your Jhola 🧺
                <span className="text-xs bg-gray-100 px-3 py-1 rounded-full text-gray-400 font-bold">
                  {totalQuantity} Items
                </span>
              </h3>
                ...
                <>
                  <div className="space-y-4 mb-10 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                    {cart.items.map((item) => (
                      <div
                        key={item._id}
                        className="flex justify-between items-center bg-gray-50/50 p-4 rounded-3xl border border-gray-100 hover:bg-white transition-colors"
                      >
                        <div className="flex flex-col">
