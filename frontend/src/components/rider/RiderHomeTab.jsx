...
function RiderHomeTab({
  ...
  //FIX: Safe PopState & Escape Key listener
  useEffect(() => {
  ...
  // FIX: Safe Date Parsing & Memoized derived state
  const activeOrders = useMemo(() => {
    ...
    return orders
      .map((order) => {
        ...
        const expireTimeMs = order.offerExpiresAt
          ? new Date(order.offerExpiresAt).getTime()
          : null;
        const safeExpireTime = Number.isFinite(expireTimeMs)
          ? expireTimeMs
          : safeCreatedAt + 60000;
  ...
  // FIX: Safe Try/Catch/Finally for Async Accept Flow
    ...
    <div className="space-y-4 pb-20">
      {!isOnline ? (
        <div className="bg-white dark:bg-gray-900 py-16 px-4 rounded-3xl text-center shadow-sm border border-gray-100 dark:border-gray-800">
          ...
          <p className="text-sm text-gray-500 mt-2 font-medium max-w-xs mx-auto">
            Toggle the switch at the top to go online and start receiving orders
            in your area.
          </p>
        </div>
      ) : isLoading ? (
        <div className="bg-white dark:bg-gray-900 py-16 px-4 rounded-3xl text-center shadow-sm border border-gray-100 dark:border-gray-800">
          ...
          <p className="font-bold text-orange-500 text-sm tracking-wide">
        ...
        <div className="bg-white dark:bg-gray-900 py-16 px-4 rounded-3xl text-center shadow-sm border border-gray-100 dark:border-gray-800 relative overflow-hidden">
          ...
          <p className="font-black text-gray-800 dark:text-gray-200 text-lg relative z-10">
            Searching your radar
          </p>
          <p className="text-sm text-gray-500 mt-2 font-medium max-w-xs mx-auto relative z-10">
            We will notify you instantly when a new order drops nearby.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          {activeOrders.map((order) => {
            ...
            return (
              <div
                key={order._id}
                className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-orange-200 dark:border-orange-500/30 flex flex-col md:flex-row md:items-center justify-between p-3 gap-3 transition-all hover:shadow-md"
              >
                ...
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  <p className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 truncate pl-4">
                    Dropoff:{" "}
                    {order.deliveryDetails?.address || "Location not provided"}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0 hide-scrollbar shrink-0 pl-4 md:pl-0">
                  ...
                  <div
                    className={`px-3 h-9 flex items-center justify-center font-black text-xs rounded-lg border w-[60px] ${isExpiring ? "bg-red-100 border-red-200 text-red-600 animate-pulse" : "bg-orange-50 border-orange-200 text-orange-600"}`}
                  >
                    {order.secondsLeft}s
                  </div>
        ...
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-md max-h-[95dvh] overflow-hidden flex flex-col shadow-2xl border border-gray-200 dark:border-gray-800 transition-all duration-300 transform scale-100">
            <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-800">
              <div>
                ...
                <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">
                  Check distance before accepting
                </p>
              </div>
              <button
            ...
            <div className="flex-1 w-full min-h-0 relative z-0 overflow-hidden">
              <MapContainer
                center={getSafeCoords(
                  previewOrder.restaurantId?.latitude,
                  previewOrder.restaurantId?.longitude,
                )}
                zoom={13}
              ...
              >
                ...
                <Marker
                  position={getSafeCoords(
                    ...
                    (previewOrder.restaurantId?.latitude || 27.505) + 0.01,
                    (previewOrder.restaurantId?.longitude || 83.669) + 0.01,
                  )}
                  icon={dropoffIcon}
                >
                  <Popup>
                    Dropoff:{" "}
                    {previewOrder.deliveryDetails?.address || "Dropoff"}
                  </Popup>
                </Marker>
              </MapContainer>
            </div>
            ...
            <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
              <button
                ...
                className={`w-full text-white py-4 rounded-xl font-black text-sm transition-all shadow-lg flex items-center justify-center gap-2 ${
                  acceptingId
                    ? "bg-gray-400 cursor-not-allowed"
                    : "bg-orange-500 hover:bg-orange-600 shadow-orange-500/30 active:scale-95"
                }`}
              >
