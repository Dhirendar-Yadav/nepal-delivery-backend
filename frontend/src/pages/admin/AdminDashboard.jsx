...
function AdminDashboard() {
  ...
  const [finHub, setFinHub] = useState({
    pendingRiderPayouts: [],
  ...
  const fetchData = useCallback(async () => {
    ...
    if (ao && Array.isArray(ao.data)) {
      const normalized = {};
      ao.data.forEach((order) => {
        normalized[order._id] = order;
      });
  ...
  const handleRestaurantOperation = async (id, data) => {
    try {
      const res = await fetch(
        `${API_BASE}/api/admin/restaurants/${id}/operate`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
  ...
  const handleRestaurantStatus = async (id, data) => {
    try {
      const res = await fetch(
        `${API_BASE}/api/admin/restaurants/${id}/status`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
      ...
      if (res.ok && result.success) {
        ...
        alert(
          `Update Failed: ${result.error || result.message || "Unknown error"}`,
        );
      }
    } catch {
  ...
  const handleSettlement = async (id, amount) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/admin/restaurants/${id}/settle`,
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  const approveBulkPayout = async (targetType) => {
    if (
      !window.confirm(`CEO Command: Settle ALL ${targetType}s in this batch?`)
    )
      return;
  ...
  const idleCount = activeOrdersArray.filter((o) =>
    isIdle(o.lastUpdate),
  ).length;
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeTab === "Orders")
      return allOrders.filter((o) => (o._id || "").toLowerCase().includes(q));
    return [];
  }, [searchQuery, activeTab, riders, sellers, customers, allOrders]);

  if (isLoading)
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center text-orange-500 font-bold tracking-[0.5em]">
        HQ INITIALIZING...
      </div>
    );
      ...
      <main className="flex-1 overflow-y-auto bg-gray-950 p-6 custom-scrollbar relative">
        {isProcessing && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="text-orange-500 font-black animate-pulse">
              SYSTEM PROCESSING... DO NOT CLOSE
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 mb-6">
          {idleCount > 0 && (
            <div className="flex-1 bg-red-600/10 border border-red-600/30 p-2.5 rounded flex items-center justify-between">
              <span className="text-red-500 font-black uppercase text-[9px]">
                🚨 {idleCount} RIDERS IDLE
              </span>
              <button
                onClick={() => setActiveTab("Live Tracking")}
                className="text-[8px] bg-red-600 text-white px-2 py-0.5 rounded"
              >
                MONITOR
              </button>
            </div>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden shadow-2xl">
          ...
          {activeTab === "Live Tracking" && (
            <LiveTrackingMap
              activeOrdersArray={activeOrdersArray}
              isIdle={isIdle}
            />
          )}
          ...
          {activeTab === "Overview" && (
            <div className="p-10 text-center space-y-4">
              ...
              <p className="text-gray-500">
                System is fully operational. Nepal logistics are live.
              </p>
              <div className="grid grid-cols-3 gap-4 max-w-3xl mx-auto pt-6">
                <StatBox
                  label="Daily Momentum"
                  value={`${stats.dailyOrders || 0} Orders`}
                />
                <StatBox
                  label="System Profit"
                  value={`NPR ${stats.netProfit || 0}`}
                />
                <StatBox
                  label="Liquid Cash"
                  value={`NPR ${stats.availableBalance || 0}`}
                />
              </div>
              ...
              <div className="mt-12 p-6 bg-purple-900/10 border border-purple-500/20 rounded-2xl max-w-xl mx-auto">
                <h3 className="text-purple-400 font-black mb-3 uppercase tracking-widest text-sm">
                  🛠️ Admin Maintenance Tools
                </h3>
                <p className="text-gray-400 mb-4">
                  Click the button below to upgrade all older Seller accounts to
                  the new active system. This fixes the "Account Disabled" login
                  error instantly.
                </p>
                <button
...
function StatBox({ label, value }) {
  return (
    <div className="p-6 bg-gray-900 border border-gray-800 rounded-2xl">
      <span className="block text-[9px] text-orange-500 font-black mb-2 uppercase">
        {label}
      </span>
