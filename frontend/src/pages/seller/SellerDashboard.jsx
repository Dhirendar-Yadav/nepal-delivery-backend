...
function Dashboard() {
  ...
  const [orderSection, setOrderSection] = useState(
    () => sessionStorage.getItem("sellerDashboardOrderSection") || null,
  );
  ...
  const { userProfile, setUserProfile, fetchUserProfile } = useSellerProfile({
    API_BASE,
  });

  const [profileImageRefreshKey, setProfileImageRefreshKey] = useState(() =>
    Date.now(),
  );
  ...
  useEffect(() => {
    sessionStorage.setItem("sellerDashboardActiveTab", activeTab);
  }, [activeTab]);

  useEffect(() => {
    if (orderSection) {
      sessionStorage.setItem("sellerDashboardOrderSection", orderSection);
    } else {
      sessionStorage.removeItem("sellerDashboardOrderSection");
    }
  }, [orderSection]);
  ...
  const handleSellerNavigationBack = useCallback((previousState) => {
    ...
    if (typeof previousState === "object" && previousState.tab) {
    ...
    if (typeof previousState === "object" && previousState.profilePhoto) {
  ...
  const navigateToOrderSection = useCallback(
    (nextSection) => {
  ...
  const orderSectionOrders = {
    ...
    out: orders.filter((order) => order.status === "Out for Delivery"),
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-3 [scrollbar-width:thin]">
            <div className="space-y-1">
              <button
                type="button"
                onClick={() =>
                  setIsOrdersDrawerOpen((previousState) => !previousState)
                }
