  ...
  const handleSavePaymentMethod = async (payload) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.message || data.error || "Unable to save payment method.",
        );
      }
    ...
    <>
      <section className="w-full max-w-2xl space-y-5 sm:space-y-6">
        <div className="flex items-center justify-between gap-3 border-b border-gray-700 pb-3 sm:gap-5 sm:pb-4">
          ...
          >
            {hasPaymentMethod ? "Edit Payment Method" : "Add Payment Method"}
          </button>
        </div>
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate text-lg font-black text-white sm:text-xl">
              {Number.isSafeInteger(restaurant?.walletBalance)
                ? `NPR ${(restaurant.walletBalance / 100).toLocaleString(
                    "en-NP",
                    {
          ...
          <div className="min-w-0 text-right">
            ...
            <p className="mt-1 truncate text-lg font-black text-white sm:text-xl">
              {Number.isSafeInteger(restaurant?.totalSettled)
                ? `NPR ${(restaurant.totalSettled / 100).toLocaleString(
                    "en-NP",
                    {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
            ...
            <div className="rounded-2xl border border-gray-700 bg-gray-800/60 px-4 py-4">
              ...
              {restaurant.payoutSettings.method === "Bank" ? (
                <div className="mt-3 space-y-2 text-xs">
                  ...
                  <p className="text-gray-300">
                    <span className="font-black text-gray-500">Bank Name</span>
                    {" — "}
                    {restaurant.payoutSettings.bankDetails?.bankName ||
                      "Not Provided"}
                  </p>
                ...
                <div className="mt-3 space-y-2 text-xs">
                  ...
                  <p className="text-gray-300">
                    ...
                    {" — "}
                    {restaurant.payoutSettings.eSewaId || "Not Provided"}
                  </p>
                </div>
              )}
            </div>
      ...
      <PaymentMethodModal
        ...
        title={hasPaymentMethod ? "Edit Payment Method" : "Add Payment Method"}
        initialMethod={
          hasPaymentMethod ? restaurant.payoutSettings.method : "Bank"
        }
        initialValues={
          restaurant?.payoutSettings?.method === "Bank"
            ? {
                            ...
                            {isOrderEarning && transaction.order && (
                              <>
                                ...
                                <span className="text-right text-gray-300">
                                  NPR{" "}
                                  {(
                                    transaction.order.foodCost / 100
                                ...
                                <span className="text-right text-gray-300">
                                  NPR{" "}
                                  {(
                                    transaction.order.platformFee / 100
                                  ).toLocaleString("en-NP", {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })}
